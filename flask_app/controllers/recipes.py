...
def editForm(id):
    ...
    if not session['user_id'] == currentRecipe['user_id']:
        flash('You cant edit this', 'noAccessError')
        return redirect('/dashboard')
   
    return render_template('updateRecipe.html', loggedUser = User.get_user_by_id(data), recipes = Recipe.get_recipe_by_id(data))
...
def updateRecipe(id):
    ...
    if not session['user_id'] == currentRecipe['user_id']:
        flash('You cant update this', 'noAccessError')
        return redirect('/dashboard')
